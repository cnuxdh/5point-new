...
void compute_nullspace_basis_pano(int n, v3_t *a, v3_t *b, double *basis) 
{
    double *Q, *S, *U, VT[81];
    int max_dim;
    int i;
    ...
    for (i = 0; i < 5; i++) 
	{
        ...
        row[2] = a[i].p[2] * b[i].p[0];

        row[3] = a[i].p[0] * b[i].p[1];
        row[4] = a[i].p[1] * b[i].p[1];
        row[5] = a[i].p[2] * b[i].p[1];

        row[6] = a[i].p[0] * b[i].p[2];
        row[7] = a[i].p[1] * b[i].p[2];
        row[8] = a[i].p[2] * b[i].p[2];
    }
...
void generate_Ematrix_hypotheses_pano(int n, v3_t *left_pts, v3_t *rt_pts, double radius, 
                                 int *num_poses, double *E) 
{
    ...
    compute_nullspace_basis_pano(n, left_pts, rt_pts, basis);
...
int evaluate_Ematrix_pano(int n, v3_t *l_pts, v3_t *r_pts, 
						  double radius, double thresh_norm,
                     	  double *F, int *best_inlier, double *score)
{
    ...
    for (i = 0; i < n; i++) {
#if 1
        v3_t r = r_pts[i]; //v3_new(Vx(r_pts[i]), Vy(r_pts[i]), 1.0);
        v3_t l = l_pts[i]; //v3_new(Vx(l_pts[i]), Vy(l_pts[i]), 1.0);
#else
...
#else
        double resid = fmatrix_compute_residual_pano(F, l, r, radius);
#endif     
...
/*  for panorama  
inputs:
	ransac_threshold:  0.25*9
	ransac_rounds:     500
outputs:
	em:  best essential matrix

*/
int compute_pose_ransac_pano(int n, v3_t *l_pts, v3_t *r_pts, double radius, 
							 ...
							 double *R_out, double *t_out)
{
	...
	for (round = 0; round < ransac_rounds; round++)
	{
		...
		v3_t r_pts_inner[5], l_pts_inner[5];
		int indices[5];
		int num_hyp;
		double E[90];
		int inliers_hyp[10];
		...
		for (i = 0; i < 5; i++) 
		{
			r_pts_inner[i] = r_pts[indices[i]];
			l_pts_inner[i] = l_pts[indices[i]];
		...
		generate_Ematrix_hypotheses_pano(5, l_pts_inner, r_pts_inner, radius, &num_hyp, E);
		...
		for (i = 0; i < num_hyp; i++) 
		{
			...
			inliers = evaluate_Ematrix_pano(n, l_pts, r_pts, radius, // r_pts_norm, l_pts_norm, 
					  thresh_norm, E2, // E + 9 * i, 
					  &best_inlier, &score);
			...
			if (inliers > max_inliers || (inliers == max_inliers && score < min_score)) 
			{
					...
					printf("max_inliers: %d  min_sorce: %lf \n", max_inliers, min_score);
			}
	...
	printf("max_inliers:%d   min_score: %lf \n  ", max_inliers, min_score);
	...
	if (max_inliers > 0) 
	{
		int best_inlier;
		double score;
		int success = find_extrinsics_essential_multipt_pano(E_best, n, 
										l_pts, r_pts, R_out, t_out);		
	}
	
	printf("best essential matrix by pano: \n");
